                        ...
                        <div class="w-32 h-32 mx-auto rounded-2xl bg-gradient-to-tr from-slate-800 to-slate-900 border-2 border-slate-700 flex items-center justify-center shadow-inner mb-4 overflow-hidden">
                            @if($profile->photo_path)
                                @php
                                    ...
                                    if (\Illuminate\Support\Str::startsWith($photoSrc, ['http://', 'https://'])) {
                                        // External URL - use as-is
                                    } elseif (\Illuminate\Support\Str::startsWith($photoSrc, '/')) {
                                        // File in public/ folder
                                        $photoSrc = asset($photoSrc);
                                    } else {
                                        // File in storage
                                        $photoSrc = asset('storage/' . $photoSrc);
                                    }
                                @endphp
                                <img src="{{ $photoSrc }}" alt="{{ $profile->full_name }}" class="w-full h-full object-cover">
                            @else
